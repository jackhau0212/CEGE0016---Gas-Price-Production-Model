...
similar to y = mx + c
'''
price_forecast_deterministic = price_model.predict(t) + historic_gas_data['Price'].iat[-1] - price_model.predict(t)[0]
...
for i in range(1, 12*30):
    price_forecast[i + 1] = price_forecast_deterministic[i] * 1 + (sigma * 1 * np.random.normal(0,1))
